...
function addToKnown(mod: Record<string, unknown> | undefined): boolean {
  if (!mod) return false
  const known = (mod as Record<string, unknown>)[KNOWN_KEY]
  if (known instanceof Set) {
    known.add(RECEIPT_EVENT_TYPE)
    return true
  }
  if (Array.isArray(known)) {
    if (!known.includes(RECEIPT_EVENT_TYPE)) known.push(RECEIPT_EVENT_TYPE)
    return true
  }
  return false
}

export function registerReceiptEventTypes(): void {
  try {
    const anchors = [import.meta.url, process.argv[1]].filter(Boolean) as string[]
    ...
    for (const anchor of anchors) {
      ...
      for (const id of CANDIDATE_MODULES) {
        ...
        try {
          ...
          if (addToKnown(mod as Record<string, unknown>)) return
          // The types-only entry may re-export the runtime record - try default too
          if (mod && typeof mod === 'object' && 'default' in mod) {
            addToKnown((mod as { default: Record<string, unknown> }).default)
          }
        } catch {
