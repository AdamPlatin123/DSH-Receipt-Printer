 ...
 */
export function buildRecapPrompt(stats: ReceiptStats): string {
  const tools = Object.entries(stats.toolCalls)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 4)
    .map(([n, c]) => `${n}x${c}`)
    .join(', ')
  const files = stats.filesWritten.slice(0, 4).map(fp =>
    String(fp).replace(/\\/g, '/').split('/').pop(),
  )
  const lines = [
    `- 项目: ${stats.project}`,
...
export async function generateRecap(
  ...
  const dbg = (msg: string) => {
    if (process.env.THERMAL_DEBUG !== '1') return
    try {
  ...
  try {
    ...
    for await (const chunk of llm.stream({ ...options, signal: ac.signal })) {
      ...
      if (chunk?.type === 'text-delta' && typeof chunk.text === 'string') {
        if (answer.length < 2048) answer += chunk.text
      }
      if (chunk?.type === 'finish' || n <= 2) {
        try { finishPayload = JSON.stringify(chunk).slice(0, 300) } catch { finishPayload = String(chunk) }
      }
    }
