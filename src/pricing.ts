...
export const DEEPSEEK_MODEL_PRICES: Record<string, RatePair> = {
  'deepseek-v4-flash': {
    inputMiss: [1.5, 3.0],
    inputHit: [0.05, 0.10],
    output: [4.5, 9.0],
...
export function priceForModel(model: string): RatePair | undefined {
  let best: RatePair | undefined
  let bestLength = 0
...
export function computeCostCny(
  ...
  return (
    (input - cacheRead) * price.inputMiss[rateIndex] +
