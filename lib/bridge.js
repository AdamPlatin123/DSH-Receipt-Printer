...
export function resolveBridgePath(config) {
    return config.bridgePath || join(PKG_ROOT, 'vendor', 'thermal_bridge.py');
}
export async function runPythonBridge(config, stats) {
    ...
    return await new Promise(resolve => {
        ...
        child.on('close', code => {
            if (code === 0)
                ...
                resolve({ ok: false, stdout, stderr, error: `bridge exit=${code}: ${stderr.trim() || '(no stderr)'}` });
        });
